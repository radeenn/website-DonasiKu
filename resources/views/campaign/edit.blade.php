...
<div class="page-hero">
    ...
    <p>Perbarui data campaign dan ganti gambar utama bila diperlukan.</p>
</div>
...
<div class="form-section">
    ...
    <form action="{{ route('campaign.update', $campaign) }}" method="POST" enctype="multipart/form-data" class="form-card">
        @csrf
        @method('PUT')
        ...
        <div class="section-box">
            ...
            <div class="form-grid">
                <div class="form-group full">
                    ...
                    <input id="title" type="text" name="title" value="{{ old('title', $campaign->title) }}" required class="form-input">
                </div>
                <div class="form-group full">
                    <label class="form-label" for="description">Deskripsi</label>
                    <textarea id="description" name="description" rows="4" required class="form-input">{{ old('description', $campaign->description) }}</textarea>
                </div>
                <div class="form-group full">
                    ...
                    @if($campaign->image)
                        <div class="current-image">
                            ...
                            <div>Gambar saat ini. Pilih gambar baru di bawah untuk menggantinya.</div>
                        </div>
                    @endif
                    <div class="upload-box" id="campaign-dropzone">
                        <input id="campaign-image" type="file" name="image" accept="image/png,image/jpeg,image/webp">
                        <div class="upload-icon">IMG</div>
                        <div class="upload-title">Klik atau tarik gambar baru ke area ini</div>
                        <div class="upload-sub">Kosongkan bila tidak ingin mengganti gambar. Maksimal 5 MB.</div>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <input id="target_donation" type="number" name="target_donation" value="{{ old('target_donation', $campaign->target_donation) }}" required min="0" class="form-input">
                </div>
                <div class="form-group">
                    <label class="form-label" for="collected_donation">Dana Terkumpul (Rp)</label>
                    <input id="collected_donation" type="number" name="collected_donation" value="{{ old('collected_donation', $campaign->collected_donation) }}" min="0" class="form-input">
                </div>
                <div class="form-group full">
                    <label class="form-label" for="deadline">Batas Waktu</label>
                    <input id="deadline" type="date" name="deadline" value="{{ old('deadline', $campaign->deadline) }}" required class="form-input">
                </div>
            </div>
        </div>
        ...
        <div class="section-box">
            ...
            <div class="form-grid">
                <div class="form-group full">
                    ...
                    <input id="bank_name" type="text" name="bank_name" value="{{ old('bank_name', $campaign->account->bank_name ?? '') }}" required class="form-input">
                </div>
                <div class="form-group">
                    <label class="form-label" for="account_number">Nomor Rekening</label>
                    <input id="account_number" type="text" name="account_number" value="{{ old('account_number', $campaign->account->account_number ?? '') }}" required class="form-input">
                </div>
                <div class="form-group">
                    <label class="form-label" for="account_holder">Nama Pemilik Rekening</label>
                    <input id="account_holder" type="text" name="account_holder" value="{{ old('account_holder', $campaign->account->account_holder ?? '') }}" required class="form-input">
                </div>
            </div>
        </div>
